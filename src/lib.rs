...
impl CocoFile {
    pub fn make_id_map(&self) -> std::collections::HashMap<u64, IDMapEntry<'_>> {
        let image_map: HashMap<u64, &CocoImage> = self.images.par_iter().progress().map(|im| (im.id, im)).collect();
        let annotation_map: HashMap<u64, Vec<&CocoAnnotation>> = self.annotations.par_iter().progress().fold(|| HashMap::new(), |mut acc, ann| {
        ...
        image_map.par_iter().progress_count(image_map.len() as u64).map(|(&id, &image)| {
            (id, IDMapEntry {
                id,
                image,
